...
class app:
    def __init__(self):
        ...
        tk.Entry(self.root,textvariable=self.currentDir,width=149).place(x=0,y=0)
        tk.Label(self.root,text="YOUR PASSWORD").place(x=10,y=30)
        tk.Entry(self.root,textvariable=self.your_password,font=('arial 20'),width=58).place(x=10,y=50)
        tk.Label(self.root,text="LENGTH:").place(x=10,y=110)
        self.stateLabel = tk.Label(self.root,text="",width=127)
        self.stateLabel.place(x=2,y=141)
        self.btnCreate = tk.Button(self.root,text="CREATE PASSWORD",width=123,height=2,bg="gray86",command=self.init_task)
        self.btnCreate.place(x=12,y=168)
        tk.Button(self.root,text="SAVE PASSWORD",width=60,height=2,bg="gray86",command=self.save_password).place(x=12,y=218)
        tk.Button(self.root,text="COPY",width=60,height=2,bg="gray86",command=self.copy).place(x=453,y=218)
        self.len=ttk.Combobox(self.root,width=5,state="readonly")
        self.len.place(x=68,y=110)
        tk.Label(self.root,text="MIN LOWERCASE:").place(x=148,y=110)
        self.min_low=ttk.Combobox(self.root,width=5,state="readonly")
        self.min_low.place(x=255,y=110)
        tk.Label(self.root,text="MIN UPPERCASE:").place(x=340,y=110)
        self.min_upp=ttk.Combobox(self.root,width=5,state="readonly")
        self.min_upp.place(x=440,y=110)
        tk.Label(self.root,text="MIN NUMBERS:").place(x=525,y=110)
        self.min_num=ttk.Combobox(self.root,width=5,state="readonly")
        self.min_num.place(x=620,y=110)
        tk.Label(self.root,text="MIN SPECIAL CHARS:").place(x=705,y=110)
